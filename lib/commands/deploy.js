							...
							fs.readdir(PROFILES_DIR, function (err, files) {
								...
								while (l--)	{
									if (files[l] !== '.DS_Store') {
										...
										var parsed_plist = plist.parseStringSync(xml);

										var PlistAppIdPrefix = parsed_plist.ApplicationIdentifierPrefix
										var PlistAppId = parsed_plist.Entitlements['application-identifier'];
										var PlistProfile = PlistAppId.replace(PlistAppIdPrefix+'.','').replace('.*','');

										// console.log(tiapp.id+' '+PlistProfile + ' '+tiapp.id.indexOf(PlistProfile));
