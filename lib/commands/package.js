		...
		shell.command('xcodebuild', ['-showsdks'], true, function (error, output) {
			...
			shell.command('security', ['find-identity', '-p', 'codesigning', '-v'], true, function (error, output) {
				...
				while (l--) {
					...
					if (line.indexOf('iPhone Distribution') !== 0) {
						continue;
					}
				...
				function waitInput (value) {
					...
					if (step === 0) {
						...
						if (typeof v === 'number' && v < profiles.length && v >= 0) {
							...
							shell.command(py, [argv.adhoc ? 'adhoc' : 'distribute', sdk, pwd, tiapp.id, tiapp.name, selectedProfile, selectedIdentity, family], false, logWatcher, function (code) {
								if (code !== 0) {
