...
function execute (argv, cfg, tiapp) {
	if (argv.ios && (argv.iphone || argv.ipad)) {
		...
		if (argv.x) {
			...
			waxsim_cmd = spawn(__dirname + '/../../bin/waxsim', [process.cwd() + '/build/iphone/build/Debug-iphonesimulator/' + tiapp.name + '.app']);
			
			// bring simulator to from
			waxsim_cmd.on('exit', function () {
