...
var path = require("path"),
	...
	externals = require('./titanium/externals/externals').externals,
...
function banner()
{
	...
	console.log();
	console.log("Welcome to ".white + "Titanium".bold.yellow + " - the open source mobile framework by".white + " *Appcelerator* ".red.red+" and contributors".white);
	console.log();
}
...
titanium.start = function(setup)
{
	...
	if (cmd === 'help') {
		require('./titanium/builtins/help.js').execute(module,environ,config.config,args,optimist.argv);
	}
	...
	{
		require('./titanium/externals/'+cmd+'.js').execute(module,environ,config.config,args,optimist.argv);
	}
	else
	{
		log.error("Couldn't find command: " + cmd.grey.bold + ".  You can run " + "titanium help ".cyan.bold + " to get a list of all commands",2);
	}
